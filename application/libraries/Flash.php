...
class Flash {
    ...
    public function __construct($config = array())
    {
        ...
        if ( ! empty($config)) {
            $this->initialize($config['flash']);
        }

        log_message('debug', 'Flash Class Initialized');
    }
    ...
    private function _add_message($message, $data = '', $type = 'default', $display_now = false)
    {
        ...
        if ($display_now === false) {
            ...
            $this->CI->session->set_flashdata($this->session_name, $this->_session_messages);
        }
        else {
    ...
    {
        $session_messages = $this->CI->session->flashdata($this->session_name);
